...
namespace OpenShadows.Data.Rendering
{
    ...
    internal static class StaticResourceCache
    {
        ...
        internal static Texture GetPinkTexture(GraphicsDevice gd, ResourceFactory factory)
        {
            if (_pinkTex == null)
            {
                RgbaByte[] pink = new RgbaByte[] { RgbaByte.Pink };
                _pinkTex = factory.CreateTexture(TextureDescription.Texture2D(1, 1, 1, 1, PixelFormat.R8_G8_B8_A8_UNorm, TextureUsage.Sampled));
                gd.UpdateTexture<RgbaByte>(_pinkTex, pink, 0, 0, 0, 1, 1, 1, 0, 0);
            }
