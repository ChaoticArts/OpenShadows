...
{
	public static class BobExtractor
    {
		public static ImageData ExtractImage(byte[] data)
		{
			...
			if (!CheckSignature(f))
			{
				throw new InvalidDataException("Not a valid BOB image");
			}
			...
			byte animCount = f.ReadByte();
			ushort h_unk1 = f.ReadUInt16();
            ushort h_unk2 = f.ReadUInt16();
		...
		private static bool CheckSignature(BinaryReader br)
		{
			...
			return b == 0x42 && o == 0x4F && b2 == 0x42 && one == 0x01;
		}
